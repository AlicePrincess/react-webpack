...
class MyComponent extends React.Component {
   ...
   componentDidMount() {
    //  token, offsetHead, offsetTail, startTime, endTime

    this.props.fetchAllTuso(this.props.user.get('token'),(this.state.current-1)*this.state.pageSize, this.state.current*this.state.pageSize,moment(defaultStartDate).format("YYYY-MM-DDTHH:mm:ssZ"),moment(defaultEndDate).format("YYYY-MM-DDTHH:mm:ssZ"));
   }
   componentWillReceiveProps(nextProps) {
     let newData=[]
     if(this.state.startValue){
       nextProps.data.map((items, index) => {
         if(Date.parse(this.state.startValue) < Date.parse(items.timestamp) && Date.parse(items.timestamp) < Date.parse(this.state.endValue)){
            newData.push(items);
         }
       })
     }else{
    ...
    onDateChange(value) {
          ...
          if (value[0] == null && value[1] == null||value[1] == ""&&value[0] =="" ) {
          ...
          //  console.log(this.props.user.get('token'),(this.state.current-1)*this.state.pageSize, this.state.current*this.state.pageSize,moment(value[0]).format("YYYY-MM-DDTHH:mm:ssZ"),moment(value[1]).format("YYYY-MM-DDTHH:mm:ssZ"));
                this.props.fetchAllTuso(this.props.user.get('token'),(this.state.current-1)*this.state.pageSize, this.state.current*this.state.pageSize,moment(value[0]).format("YYYY-MM-DDTHH:mm:ssZ"),moment(value[1]).format("YYYY-MM-DDTHH:mm:ssZ"));
          }
  this.setState({searchToday:false})
    }
    /*
     预览模态框显示
     */
...
handleChangePage(data) {
  ...
  if(this.state.startValue&&this.state.endValue&&this.state.endValue!=null&&this.state.startValue!=null){
    this.props.fetchAllTuso(this.props.user.get('token'),(data.current-1)*this.state.pageSize, data.current*this.state.pageSize,moment(this.state.startValue).format("YYYY-MM-DDTHH:mm:ssZ"),moment(this.state.endValue).format("YYYY-MM-DDTHH:mm:ssZ"));

  }else{
    console.log(this.props.user.get('token'),(data.current-1)*this.state.pageSize, data.current*this.state.pageSize,moment(defaultStartDate).format("YYYY-MM-DDTHH:mm:ssZ"),moment(defaultEndDate).format("YYYY-MM-DDTHH:mm:ssZ"));
    this.props.fetchAllTuso(this.props.user.get('token'),(data.current-1)*this.state.pageSize, data.current*this.state.pageSize,moment(defaultStartDate).format("YYYY-MM-DDTHH:mm:ssZ"),moment(defaultEndDate).format("YYYY-MM-DDTHH:mm:ssZ"));
  }
        ...
        const columns = [
            {
                ...
                render: (text,record) => {
                  if(text&&text.length>0){
                    ...
                    return <a className="coverBox" >{"暂无图片"}</a>
                ...
                key: 'user',
                width: '10%',
                render: (text) => <span>{text?text.id:"暂无发布人"}</span>
            }, {
                title: '发布人昵称',
                dataIndex: 'user',
                key: 'user.nickname',
                width: '13%',
                render: (text) => <span>{text?text.nickname:"暂无发布人"}</span>
            }
        ];
        return (
