...
import {createPuppet,deletePuppet,resetPassword,fetchAllPuppet} from '../../actions/puppet'
...
class MyComponent extends React.Component {
  ...
  componentDidMount(){
    this.props.fetchAllPuppet(this.props.user.get('token'),(this.state.current-1)*this.state.pageSize, this.state.current*this.state.pageSize,moment(defaultStartDate).format("YYYY-MM-DDTHH:mm:ssZ"),moment(defaultEndDate).format("YYYY-MM-DDTHH:mm:ssZ"))
  }
  componentWillReceiveProps(nextProps){
   ...
   if(localStorage.getItem('puppetId')){
     nextProps.data.map((items, index) => {
       if(localStorage.getItem('puppetId')==items.id){
         items.status=true;
       }else{
         items.status=false;
       }
    ...
    this.setState({data:nextProps.data.toJS(),dateformat:nextProps.data.toJS(),searchOleData:nextProps.data.toJS(),selectedRowKeys: [], allCount:nextProps.allCount})
    ...
    const hasSelected = selectedRowKeys.length > 0;
      const columns = [
        ...
        // },
        {
            ...
            render: (text, record) =>
            {
                return     <Switch defaultChecked={text} checked={record.status} onChange={this.handleChangeStatus.bind(this,record)} />
                ;
            }
        }, {
            title: '操作',
...
function mapStateToProps(state) {
    return {
          ...
          allCount:state.getIn(["puppet"]).get("allCount"),
    }
}
